...
class VideoController extends Controller
{
    ...
    public function topnew(){
        $topvideo= Video::orderBy('created_at', 'desc')->take(3)->where('valide',1)->get();
        $newvideo= Video::orderBy('created_at', 'desc')->take(5)->where('valide',1)->get();
   ...
   public function voir($id){
    ...
    public function show(Request $request){
        $q = $request->input('q');
    $videos = Video::where('titre','LIKE','%'.$q.'%')->get();
    if(count($videos) > 0){
        return view('favio.recherche',['video'=>$videos,'q'=>$q]);
    }
    else {
         $errors='Aucune video de cette recherche!!';
        return view('favio.recherche',['erro'=>$errors,'q'=>$q]); 
        }
    }
